// src/utils/functions/firebaseTranslateError.ts
...
const handleTranslateFbError = (errorCode: string) => {
  switch (errorCode) {
...
const handleAuthErrors = (errorCode: string) => {
  switch (errorCode) {
    ...
    case 'vehicle-already-exists':
      return 'O veículo já está cadastrado.'
    case 'cnh-already-exists':
      return 'A CNH já está cadastrada.'
    case 'plate-already-exists':
      return 'A placa do veículo já está cadastrada.'
    case 'account-deleted':
      return 'Esta conta foi excluída. Entre em contato com o suporte para reativá-la.'
    default:
      return 'Ocorreu um erro durante o cadastro.'
  }
}

export { handleTranslateFbError, handleAuthErrors }
